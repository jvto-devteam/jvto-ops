...
| `check-live-tokens` | Manual / pre-ship only — never wired to a hook. It compares ekosistem's rendered prose against jvto-web's page files, so it needs both repos and a fresh `render:web-content`; running it from a per-edit hook would report on stale output. | A route whose ekosistem prose contains a live-number token (`{GOOGLE_RATING}`, `{PACKAGE_COUNT_SURABAYA}`, …) rendered by a jvto-web page that resolves it neither through `applyLiveNumbers` nor by naming it (warning — the token reaches the reader as literal text), and separately a page that resolves tokens by hand instead (warning — does not leak today, but only covers the tokens it names). Warn-only: presence of `applyLiveNumbers` is not proof of coverage, so it reports the class it can prove and stays quiet about the rest. |
